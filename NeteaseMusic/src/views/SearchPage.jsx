  ...
  return (
    <div className="container">
      <div className={styles.search_page}>
        ...
        <div className={styles.search_suggest}>
          {showSuggest ? <div className={styles.suggest_cont}>{
            props.search.searchSuggest.map((item, index) => {
              return <p className={styles.suggest_item} key={index}>{item.name}</p>
            })
          }</div> : null}
        </div>
        {showHots ?
            ...
            <div className={styles.hot_cont}>
              {
                props.search.searchHot.map((item, index) => {
                  return <span className={styles.hot_tag} key={index}>{item.first}</span>
                })
              }
            </div>
          </div> : null
